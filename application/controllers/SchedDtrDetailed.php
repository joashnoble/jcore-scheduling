...
class SchedDtrDetailed extends CORE_Controller
{
    ...
    public function index() {
        ...
        $this->load->view('scheddtr_detailed_view', $data);
    }


    function schedule($schedule=null,$filter_value=null,$filter_value2=null,$type=null){

        switch($schedule){

            case 'sched-dtr-detailed': //
                        $data['period_days']=$this->RefPayPeriod_model->get_list('pay_period_id='.$filter_value);   
                        $company_setup=$this->GeneralSettings_model->get_list();
                        $data['company']=$company_setup[0];
