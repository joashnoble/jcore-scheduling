...
class ExpiringPersonnel extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_expiringpersonnel_view') == 0 || $this->session->userdata('right_expiringpersonnel_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['title'] = 'Expiring Personnel Report';

        $this->load->view('expiring_personnel_view', $data);
    }
