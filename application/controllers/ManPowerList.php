...
class ManPowerList extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_manpower_view') == 0 || $this->session->userdata('right_manpower_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['title'] = 'Man Power List (Active)';

        $this->load->view('manpower_list_view', $data);
