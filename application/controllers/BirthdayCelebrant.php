...
class BirthdayCelebrant extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_birthdaycelebrant_view') == 0 || $this->session->userdata('right_birthdaycelebrant_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['title'] = 'Birthday Celebrant Report';

        $this->load->view('birthday_celebrant_view', $data);
    }
