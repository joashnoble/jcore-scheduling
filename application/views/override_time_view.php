                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="panel_details">
                                        <div class="row" id="divemployeedetails">
                                            <div class="col-md-4">
                                                <center>
                                                <img id="image_name" src="<?php echo $this->config->item('base_urlmain').'/assets/img/anonymous-icon.png';?>"></img>
                                            </center>
                                            </div>
                                            <div class="col-md-6">
                                                <div style="margin-top: 40px; margin-left: 20px;width: 100%;">
                                        ...
                                        <div class="row" style="margin-bottom: 10px;margin-top: 30px;">
                                            <div class="form-group">
                                                <span class="col-sm-3 inlinecustomlabel-sm" for="inputEmail1">
                                                    Employee
                                                </span>
                                                <div class="col-sm-8">
                                  ...
                                  <label for="tab1">Time In/Out</label>
                                    
                                  <input id="tab2" type="radio" name="tabs">
                                  <label for="tab2">Break In/Out</label>
                                    
<!--                                   <input id="tab3" type="radio" name="tabs">
                                  <label for="tab3">OT OUT</label> -->
                                  ...
                                  <section id="content1">
                                        ...
                                        <div class="row">
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <input type="text" name="clock_in_date" class="date-picker form-control clock_in_date" readonly>
                                                </div>
                                                <div class="col-sm-4">
                                        ...
                                        <div class="row">
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <input type="text" name="clock_out_date" class="date-picker form-control clock_out_date" readonly>
                                                </div>
                                                <div class="col-sm-4">
                                            ...
                                            <div class="row">
                                              ...
                                              <section id="content2">
                                                <form id="frm_break_list">
                                                    <div id="break_list" style="overflow: auto;overflow-x:hidden; height:260px;width: 100%;padding: 10px;"></div>
                                                    <div id="update_panel">
                                                        <hr>
                                                        <div class="row"><div style="float: right;margin-right: 15px;"><button type="button" id="btn_update_break" class="btn btn-primary" style="padding: 10px;background-color: #CFD8DC !important; color: black;"><i class="fa fa-edit"></i> Update</button></div></div>
                                                    </div>
                                                </form>
                                              </section>
...
$(document).ready(function(){
    ...
    $('#hremp').hide();
    $('#break_list').html('<center>No Data Available</center>');
    $('#update_panel').hide();

    $('#ot_list').html('<center>No Data Available</center>');
    $('#ot_panel').hide();
    ...
    var bindEventHandlers=(function(){
        ...
        $('#btn_update_break').click(function(){
                if (validateRequiredFields($('#frm_break_list'))){
        ...
        var selectsched = function(){
            ...
            selectscheduleid().done(function(response){
                    if (response.data.length > 0){
                        ...
                        $('.clock_in_date').val(response.data[0].time_in_date);
                        $('.clock_out_date').val(response.data[0].time_out_date);
                        ...
                        var schedule_employee_id = response.data[0].schedule_employee_id;
                        selectschedbreak(schedule_employee_id).done(function(response){
                            var rows = response.data;
                            if (rows.length > 0){

                                $('#break_list').html('');
                                $.each(rows,function(i,value){
                                    $('#break_list').append(newRowItem({
                                        break_time : value.break_time,
                                        break_out : value.break_out,
                                        date_break_out : value.date_break_out,
                                        date_break_in : value.date_break_in,
                                        break_in : value.break_in,
                                ...
                                $('#update_panel').show();
                                process_btn_cancel_break();
                            }else{
                                $('#break_list').html('');
                                $('#break_list').html('<center>No Break Available</center>');
                                $('#update_panel').hide();
                            }
                        });
     ...
     var newRowItem=function(d){
        return    '<div class="row">'+
                 '<input type="hidden" name="employee_break_id[]" value="'+d.employee_break_id+'">'+
                  '<div style="background: #CFD8DC; color: black; font-weight: bold; border: 1px solid #546E7A;padding: 5px; margin: 10px; margin-right: 15px; margin-left: 15px;">'+d.break_time+'<input type="button" style="float: right;margin-top: -3px; font-size:8pt;" name="cancel_break" value="Cancel Break" class="btn btn-danger" id="'+d.employee_break_id+'"></input></div>'+
                  '<div style="border: 1px solid #546E7A;padding: 5px; margin: 10px; margin-right: 15px; margin-left: 15px;margin-top:-5px;">'+
                  '<div class="form-group">'+
                  '<span class="col-sm-3 inlinecustomlabel-sm" for="inputEmail1">Break Out</span>'+
                  '<div class="col-sm-4">'+
                  '<input type="text" name="date_break_out[]" class="date'+d.employee_break_id+' date-picker form-control" value="'+d.date_break_out+'" data-error-msg="All Fields are required." required>'+
                  '</div>'+
                  '<div class="col-sm-4">'+
                  '<input type="text" name="time_break_out[]" class="time'+d.employee_break_id+' timepicker2 form-control" value="'+d.break_out+'" data-error-msg="All Fields are required." required>'+
                  '</div>'+
                  '</div>'+
                  '<div class="form-group">'+
                  '<span class="col-sm-3 inlinecustomlabel-sm" for="inputEmail1">Break In</span>'+
                  '<div class="col-sm-4">'+
                  '<input type="text" name="date_break_in[]" class="date'+d.employee_break_id+' date-picker form-control" value="'+d.date_break_in+'" data-error-msg="All Fields are required." required>'+
                  '</div>'+
                  '<div class="col-sm-4">'+
                  '<input type="text" name="time_break_in[]" class="time'+d.employee_break_id+' timepicker2 form-control" value="'+d.break_in+'" data-error-msg="All Fields are required." required>'+
                  '</div>'+
                  '</div>'+
                  '</div>'+
                  '</div>';
...
function process_btn_cancel_break(){

        $('input[name^="cancel_break"]').click(function(){
            var employee_break_id = $(this).attr('id');

            $('.date'+employee_break_id).each(function(){

                $('.date'+employee_break_id).val('0000-00-00');

            });

            $('.time'+employee_break_id).each(function(){

                $('.time'+employee_break_id).val('0:00:00');
        ...
        var changeemployeedetails = function(){
            getemployeedetails().done(function(response){
                if (response.employee_list.length > 0){
                    $('#image_name').attr('src','<?php echo $this->config->item('base_urlmain');?>/'+response.employee_list[0].image_name);
        ...
        var updateBreak=function(){
            var _data = $('#frm_break_list').serializeArray();
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"SchedEmployee/transaction/override_break",
                "data":_data,
                "beforeSend": showSpinningProgressLoading($('#'))
            });
        };        
        ...
        var selectschedbreak=function(schedule_employee_id){
            var _data=$('#').serializeArray();
            _data.push({name : "schedule_employee_id" ,value : schedule_employee_id});
            return $.ajax({
                "dataType":"json",
                "type":"POST",
